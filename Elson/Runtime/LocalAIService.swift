...
struct LocalAIService: Sendable {
    ...
    func runIntentAgent(
        ...
        geminiAPIKey: String
    ) async throws -> AudioIntentDecisionResult {
        switch provider {
        case .google:
            return try await runGeminiIntentAgent(
                request: envelope,
                allowFullAgent: allowFullAgent,
                geminiAPIKey: geminiAPIKey
            )
        case .cerebras:
            return try await runCerebrasIntentAgent(
                request: envelope,
                allowFullAgent: allowFullAgent,
    ...
    private func runCerebrasIntentAgent(
        request envelope: ElsonRequestEnvelope,
        allowFullAgent: Bool,
        cerebrasAPIKey: String
    ) async throws -> AudioIntentDecisionResult {
        ...
        let trimmedRawTranscript = (envelope.rawTranscript ?? envelope.enhancedTranscript)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedRawTranscript.isEmpty else {
            return AudioIntentDecisionResult(
                route: .directTranscript,
                reason: "Empty audio transcript.",
                threadDecision: envelope.continuationContext?.hasCandidateThread == true ? .continueCurrentThread : .startNewThread,
                replyRelation: .none,
                replyConfidence: nil
            )
        }

        let stage = ModelConfig.shared.localRuntime.cerebras.intentAgent
        let model = stage.model
        let startedAt = Date()
        ...
        let systemPrompt = ElsonPromptCatalog.intentAgentSystemPrompt(
            intentAgentPrompt: envelope.intentAgentPrompt,
            includeConversationHistory: envelope.surface == "chat"
        )
        let userPrompt = ElsonPromptCatalog.intentAgentUserPrompt(
            envelope: envelope,
            attachmentSummary: attachmentSummary,
            fullAgentAllowed: allowFullAgent
        )
        let cerebrasAPIURL = "https://api.cerebras.ai/v1/chat/completions"

        let url = URL(string: cerebrasAPIURL)!
        ...
        var payload: [String: Any] = [
            "model": model,
            "messages": messages,
            "temperature": stage.temperature ?? 0,
            "response_format": [
                "type": "json_object"
            ],
        ]
        ...
        await IntentEvalFixtureStore.shared.captureIntentFixture(
            envelope: envelope,
            provider: .cerebras,
            stage: stage,
            allowFullAgent: allowFullAgent,
        ...
        DebugLog.providerEvent(
            phase: "start",
            service: "cerebras_intent_agent",
            model: model,
            metadata: "provider=cerebras request_id=\(envelope.requestId) thread_id=\(envelope.threadId) surface=\(envelope.surface) input_source=\(envelope.inputSource) raw_chars=\(trimmedRawTranscript.count) snippet_count=\(envelope.transcriptSnippetCount ?? 0) full_agent_allowed=\(allowFullAgent)",
            payloadPreview: requestPreview
        )
        ...
        do {
            ...
            let rawBody = rawBodyText(from: data)
            do {
                try validate(response: response, data: data, service: "Cerebras Intent Agent")
            } catch {
                DebugLog.providerFailure(
                    service: "cerebras_intent_agent",
                    model: model,
            ...
            let extracted = try extractedResponseText(from: data, service: "Cerebras Intent Agent")
            DebugLog.providerEvent(
                phase: "parse_extracted",
                service: "cerebras_intent_agent",
                model: model,
                metadata: "provider=cerebras request_id=\(envelope.requestId) thread_id=\(envelope.threadId) source=\(extracted.source) duration_ms=\(durationMS(since: startedAt))",
                payloadPreview: extracted.text
            )

            guard let jsonData = extractJSONData(from: extracted.text) else {
                let error = LocalAIServiceError.invalidResponse("Cerebras Intent Agent")
                DebugLog.providerFailure(
                    service: "cerebras_intent_agent",
                    model: model,
            ...
            let rawDecision = try JSONDecoder().decode(LocalAudioIntentResponse.self, from: jsonData)
            let route = normalizedAudioDeciderRoute(rawDecision.route, allowFullAgent: allowFullAgent)
            let defaultThreadDecision: AudioDeciderThreadDecision = envelope.continuationContext?.hasCandidateThread == true
                ? .continueCurrentThread
                : .startNewThread
            let threadDecision = normalizedAudioDeciderThreadDecision(
                rawDecision.threadDecision,
                defaultValue: defaultThreadDecision
            )
            let replyRelation = normalizedAudioDeciderReplyRelation(rawDecision.replyRelation)
            let replyConfidence = normalizedReplyConfidence(rawDecision.replyConfidence)
            let reason = rawDecision.reason?.trimmingCharacters(in: .whitespacesAndNewlines)
                .nilIfEmpty ?? defaultAudioDeciderReason(for: route)

            DebugLog.providerEvent(
                phase: "success",
                service: "cerebras_intent_agent",
                model: model,
                metadata: "provider=cerebras request_id=\(envelope.requestId) thread_id=\(envelope.threadId) surface=\(envelope.surface) input_source=\(envelope.inputSource) duration_ms=\(durationMS(since: startedAt)) route=\(route.rawValue) reason=\(reason)",
                payloadPreview: """
                response_source: \(extracted.source)

                route:
                \(route.rawValue)

                thread_decision:
                \(threadDecision.rawValue)

                reply_relation:
                \(replyRelation.rawValue)
                """
            )
            return AudioIntentDecisionResult(
                route: route,
    ...
    private func runGeminiIntentAgent(
        request envelope: ElsonRequestEnvelope,
        allowFullAgent: Bool,
        geminiAPIKey: String
    ) async throws -> AudioIntentDecisionResult {
        ...
        let stage = ModelConfig.shared.localRuntime.google.intentAgent
        let model = stage.model
        let startedAt = Date()
        let attachmentSummary = envelope.attachments.isEmpty
            ? "None"
        ...
        let requestBody = makeGeminiGenerateContentBody(
            ...
            currentUserPrompt: userPrompt,
            images: images,
            responseSchema: intentAgentResponseSchema(),
            stage: stage
        )
        ...
        let geminiAPIURL = "https://generativelanguage.googleapis.com/v1beta/models/\(model):generateContent"
        ...
        await IntentEvalFixtureStore.shared.captureIntentFixture(
            ...
            systemPrompt: systemPrompt,
            userPrompt: userPrompt,
            requestPayloadData: requestBodyData,
            providerAPIURL: geminiAPIURL
        )

        DebugLog.providerEvent(
            phase: "start",
            service: "gemini_intent_agent",
            model: model,
            metadata: "provider=google request_id=\(envelope.requestId) thread_id=\(envelope.threadId) surface=\(envelope.surface) input_source=\(envelope.inputSource) raw_chars=\(trimmedRawTranscript.count) snippet_count=\(envelope.transcriptSnippetCount ?? 0) full_agent_allowed=\(allowFullAgent) images=\(images.count)",
            payloadPreview: """
            system:
            \(systemPrompt)
        ...
        do {
            let (data, response) = try await performGeminiRequest(
                ...
                requestBody: requestBody,
                model: model,
                service: "Gemini Intent Agent"
            )
            let rawBody = rawBodyText(from: data)
            do {
                try validate(response: response, data: data, service: "Gemini Intent Agent")
            } catch {
                DebugLog.providerFailure(
                    service: "gemini_intent_agent",
                    model: model,
            ...
            let extracted = try extractedResponseText(from: data, service: "Gemini Intent Agent")
            DebugLog.providerEvent(
                phase: "parse_extracted",
                service: "gemini_intent_agent",
                model: model,
                metadata: "provider=google request_id=\(envelope.requestId) thread_id=\(envelope.threadId) source=\(extracted.source) duration_ms=\(durationMS(since: startedAt))",
                payloadPreview: extracted.text
            )

            guard let jsonData = extractJSONData(from: extracted.text) else {
                let error = LocalAIServiceError.invalidResponse("Gemini Intent Agent")
                DebugLog.providerFailure(
                    service: "gemini_intent_agent",
                    model: model,
            ...
            DebugLog.providerEvent(
                ...
                service: "gemini_intent_agent",
                model: model,
                metadata: "provider=google request_id=\(envelope.requestId) thread_id=\(envelope.threadId) surface=\(envelope.surface) input_source=\(envelope.inputSource) duration_ms=\(durationMS(since: startedAt)) route=\(route.rawValue) reason=\(reason)",
                payloadPreview: """
                response_source: \(extracted.source)
    ...
    private func intentAgentResponseSchema() -> [String: Any] {
        [
            "type": "object",
            "properties": [
                "route": [
                    "type": "string",
                    "enum": [AudioDeciderRoute.directTranscript.rawValue, AudioDeciderRoute.fullAgent.rawValue]
                ],
                "thread_decision": [
                    "type": "string",
                    "enum": AudioDeciderThreadDecision.allCases.map(\.rawValue)
                ],
                "reply_relation": [
                    "type": "string",
                    "enum": AudioDeciderReplyRelation.allCases.map(\.rawValue)
                ],
                "reply_confidence": [
                    "type": ["number", "null"],
